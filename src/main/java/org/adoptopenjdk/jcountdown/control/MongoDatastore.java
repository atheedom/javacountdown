...
public class MongoDatastore {
    ...
    /**
     * This method finds the nearest geo position based on the given
     * latitude/longitude. Refer to http://www.maxmind.com/en/worldcities for
     * the data behind it.
     *
     * @param latitude
     *            The latitude
    ...
    public GeoPosition findGeoPosition(double latitude, double longitude) {
        return datastore.find(GeoPosition.class).field("location.coordinates").near(GeoJson.point(latitude, longitude)).limit(1).get();
    }
    ...
    /**
     * Returns the data used to generate the world map of JDK 7 (or later)
     * adoption.
     */
    public Map<String, Integer> getJdkAdoption() {
        ...
        Map<String, Integer> countryPercentageAdoption = adoptionByCountry.stream().filter(a -> a.getCountry() != null)
